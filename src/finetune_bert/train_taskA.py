...
def train(args):
    ...
    logger.info(f"Task A Best Dev Accuracy: {best_acc:.4f}")
...
def load_args_from_config(config_path):
    ...
    args = argparse.Namespace(
        ...
        train_data=to_project_path(cfg["taskA"]["train_data"]),
        train_answer=to_project_path(cfg["taskA"]["train_answer"]),
        dev_data=to_project_path(cfg["taskA"]["dev_data"]),
        dev_answer=to_project_path(cfg["taskA"]["dev_answer"]),
        ...
        weight_decay=cfg["taskA"].get("weight_decay", 0.01),
        warmup_ratio=cfg["taskA"].get("warmup_ratio", 0.1),
        max_grad_norm=cfg["taskA"].get("max_grad_norm", 1.0),
        log_steps=cfg["taskA"].get("log_steps", 50),
    )
